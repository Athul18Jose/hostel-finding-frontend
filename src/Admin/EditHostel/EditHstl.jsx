...
import { getOneHstl, updateHstl } from '../../Services/allAPI'
...
function EditHstl() {
  ...
  const handleUpdate = async(e)=>{
    ...
    const { owner, phn, hstlname,category, minRent, desc, address,
        If, Iwf, IIf, IIwf, IVf, IVwf, Vf, Vwf, img1, img2, img3, img4, img5 } = hstlDetaills
  
      if (!owner || !phn || !hstlname ||!category || !minRent || !desc || !address ||
        !If || !Iwf || !IIf || !IIwf || !IVf || !IVwf || !Vf || !Vwf || !img1 || !img2 || !img3 || !img4 || !img5) {
        alert("All fields are required")
      }
      else {
        const result = await updateHstl(id,hstlDetaills) 
        console.log(result);
        if(result.status === 200){
          alert(`${result.data.hstlname} Hostel Updated Succesfull!`)
          navigate('/admin/manage-hostel')
        }
        else{
            alert('Something went wrong while updating hostel');
        }
    }
  }
  ...
  return (
    ...
    <h1 className='title text-center my-4 fw-bold'>Edit Hostel Details</h1>
      <Container>
        <div>
          ...
          <div className="d-grid gap-2 m-5">
            <MDBBtn onClick={handleUpdate} style={{ backgroundColor: 'rgb(252, 128, 3)', color: 'white' }}>Update Hostel Details</MDBBtn>
          </div>
        </div>
      </Container>
    </>
